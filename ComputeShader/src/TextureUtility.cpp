...
bool CreateTextureFromFile(GpuImage& outImage, std::filesystem::path filePath, VkImageUsageFlags usage, uint32_t mipmapCount)
{
  ...
  return CreateTextureFromMemory(outImage, fileData.data(), fileData.size(), usage, mipmapCount);
}
...
bool CreateTextureFromMemory(GpuImage& outImage, const void* srcBuffer, size_t bufferSize, VkImageUsageFlags usage, uint32_t mipmapCount)
{
  ...
  if (mipmapCount == 0)
  {
    ...
    mipmapCount = uint32_t(floor(log2(max(imageWidth, imageHeight))) + 1);
  }
  // テクスチャを生成する.
  outImage = gfxDevice->CreateImage2D(
    uint32_t(imageWidth), uint32_t(imageHeight), VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_SAMPLED_BIT | usage, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, mipmapCount);
